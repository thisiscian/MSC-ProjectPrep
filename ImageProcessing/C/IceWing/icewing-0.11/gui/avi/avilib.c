 ...
 *  The only intention is to write out RAW/MJPEG encoded
...
static int avi_add_chunk (avi_t *AVI, const uint8_t *tag, uint8_t *data, int length)
{
   ...
   if (write(AVI->fdes,c,8) != 8 ||
       (data && write(AVI->fdes,data,length) != length))
   {
...
void AVI_set_video (avi_t *AVI, int width, int height, double fps, unsigned int compressor)
{
...
static int avi_close_output_file (avi_t *AVI)
{
   ...
   int ret, njunk, sampsize, ms_per_frame, idxerror, flag;
...
#define AVIF_COPYRIGHTED        0x00020000
   ...
   if (!idxerror) flag |= AVIF_HASINDEX;
   if (!idxerror && AVI->must_use_index) flag |= AVIF_MUSTUSEINDEX;
   OUTLONG(flag);               /* Flags */
   ...
   OUT4CC ("vids");             /* Type */
   OUTLONG(AVI->compressor);    /* Handler */
   OUTLONG(0);                  /* Flags */
   ...
   OUTLONG (AVI->compressor);   /* Compression */
...
*/
static int avi_write_data (avi_t *AVI, uint8_t *data, long bytes, int audio)
{
   int n;
   uint8_t *tag;

   /* Check for maximum file length */
   if ((AVI->pos + 8 + bytes + 8 + (AVI->n_idx+1)*16) > AVI_MAX_LEN) {
   ...
   if (audio) {
      tag = (uint8_t*)"01wb";
      n = avi_add_index_entry (AVI,tag,0x00,AVI->pos,bytes);
   } else {
      tag = (uint8_t*)"00db";
      n = avi_add_index_entry (AVI,tag,0x10,AVI->pos,bytes);
   }
   if (n) return -1;

   /* Output tag and data */
   n = avi_add_chunk (AVI,tag,data,bytes);
   if (n) return -1;

   return 0;
}

int AVI_write_frame_start (avi_t *AVI, long length)
{
   ...
   if (avi_write_data(AVI,NULL,length,0)) return -1;
   return 0;
}

int AVI_write_frame (avi_t *AVI, uint8_t *data, long bytes)
{
   if (write (AVI->fdes, data, bytes) != bytes)
   {
      lseek (AVI->fdes, AVI->cur_pos, SEEK_SET);
      AVI->pos = AVI->cur_pos;
      AVI_errno = AVI_ERR_WRITE;
      return -1;
   }
   AVI->cur_written += bytes;
   return 0;
}

void AVI_write_frame_end (avi_t *AVI)
{
   /* Check for a missing pad byte / not written bytes */
   if (PAD_EVEN(AVI->cur_len) > AVI->cur_written) {
      long bytes = PAD_EVEN(AVI->cur_len) - AVI->cur_written;
      lseek (AVI->fdes, bytes, SEEK_CUR);
   }
   AVI->last_pos = AVI->cur_pos;
   AVI->last_len = AVI->cur_len;
   AVI->video_frames++;
}
...
int AVI_dup_frame (avi_t *AVI)
{
   ...
   if (avi_add_index_entry (AVI,(const uint8_t*)"00db",0x10,AVI->last_pos,AVI->last_len))
       return -1;
   AVI->video_frames++;
...
avi_t *AVI_open_input_file (const char *filename, int getIndex)
{
   ...
   for (i=0; i<hdrl_len;)
   {
      ...
      if (strncasecmp ((char*)hdrl_data+i,"strh",4) == 0)
      {
         ...
         {
            AVI->compressor = str2ulong (hdrl_data+i+4);
...
unsigned int AVI_video_compressor (avi_t *AVI)
{
...
void AVI_print_error (const char *str)
{
   ...
   fprintf (stderr, "%s: %s",str,avi_errors[aerrno]);
