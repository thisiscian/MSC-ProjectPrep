...
/* Define a Huffman table */
{
  ...
  if (nsymbols < 1 || nsymbols > 256) {
    fprintf (stderr, "jpegutils.c:  add_huff_table failed badly.\n");
	exit (1);
  }
...
static void guarantee_huff_tables(j_decompress_ptr dinfo)
{
  if ( (dinfo->dc_huff_tbl_ptrs[0] == NULL) &&
    ...
    fprintf (stderr, "Generating standard Huffman tables for this frame.\n");
                     ...
                     unsigned char *raw2)
{
   ...
   if ((width>4096)||(height>4096)) {
      fprintf (stderr, "Image dimensions (%dx%d) exceed lavtools' max (4096x4096)\n", width, height);
      goto ERR_EXIT;
   }
